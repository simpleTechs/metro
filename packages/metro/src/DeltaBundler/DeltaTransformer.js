...
class DeltaTransformer extends EventEmitter {
  ...
  bundleOptions)
  {
    ...
    this._getModuleId = this._bundleOptions.isolateModuleIDs ? (bundleOptions.createModuleIdFactory || createModuleIdFactory)() : globalCreateModuleId;this._deltaCalculator.on('change', this._onFileChange);}static create(bundler, options, bundleOptions) {return _asyncToGenerator(function* () {const dependencyGraph = yield bundler.getDependencyGraph();const deltaCalculator = new DeltaCalculator(bundler, dependencyGraph, bundleOptions);return new DeltaTransformer(bundler, dependencyGraph, deltaCalculator, options, bundleOptions);})();} /**
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             */end() {this.removeAllListeners();return this._deltaCalculator.end();} /**
        ...
        // getDelta() to initialize it.
        yield _this2._getDelta({ reset: false });}const dependencyEdges = _this2._deltaCalculator.getDependencyEdges();const output = new Map();for (const _ref of dependencyEdges.entries()) {var _ref2 = _slicedToArray(_ref, 2);const path = _ref2[0];const inverseDependencies = _ref2[1].inverseDependencies;output.set(_this2._getModuleId(path), Array.from(inverseDependencies).map(function (dep) {return _this2._getModuleId(dep);}));}return output;})();} /**
                                                                                                                                                                                                                                                                                                                                                                                                                                                                       ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                       */getDelta(sequenceId) {var _this3 = this;return _asyncToGenerator(function* () {// If the passed sequenceId is different than the last calculated one,
      // return a reset delta (since that means that the client is desynchronized)
      const reset = !!_this3._lastSequenceId && sequenceId !== _this3._lastSequenceId; // If there is already a build in progress, wait until it finish to start
      // processing a new one (delta transformer doesn't support concurrent
      // builds).
      if (_this3._currentBuildPromise) {yield _this3._currentBuildPromise;}_this3._currentBuildPromise = _this3._getDelta({ reset });let result;try {result = yield _this3._currentBuildPromise;} finally {_this3._currentBuildPromise = null;}return result;})();}_getDelta(_ref3) {var _this4 = this;let resetDelta = _ref3.reset;return _asyncToGenerator(function* () {// Calculate the delta of modules.
      var _ref4 = yield _this4._deltaCalculator.getDelta({ reset: resetDelta });const modified = _ref4.modified,deleted = _ref4.deleted,reset = _ref4.reset;const transformerOptions = yield _this4._deltaCalculator.getTransformerOptions();const dependencyEdges = _this4._deltaCalculator.getDependencyEdges(); // Return the source code that gets prepended to all the modules. This
      // contains polyfills and startup code (like the require() implementation).
      const prependSources = reset ? yield _this4._getPrepend(transformerOptions, dependencyEdges) : new Map(); // Precalculate all module ids sequentially. We do this to be sure that the
      // mapping between module -> moduleId is deterministic between runs.
      const modules = Array.from(modified.values());modules.forEach(function (module) {return _this4._getModuleId(module.path);}); // Get the transformed source code of each modified/added module.
      const modifiedDelta = yield _this4._transformModules(modules, transformerOptions, dependencyEdges);deleted.forEach(function (id) {modifiedDelta.set(_this4._getModuleId(id), null);}); // Return the source code that gets appended to all the modules. This
      // contains the require() calls to startup the execution of the modules.
      const appendSources = reset ? yield _this4._getAppend(dependencyEdges) : new Map(); // generate a random
      _this4._lastSequenceId = crypto.randomBytes(8).toString('hex');return { pre: prependSources, post: appendSources, delta: modifiedDelta, reset, id: _this4._lastSequenceId };})();}_getDeps(path, edges, deps) {if (deps.has(path)) {return deps;}const edge = edges.get(path);if (!edge) {return deps;}deps.add(path);for (const _ref5 of edge.dependencies) {var _ref6 = _slicedToArray(_ref5, 2);const dependencyPath = _ref6[1];this._getDeps(dependencyPath, edges, deps);}return deps;}_getPrepend(transformOptions, dependencyEdges) {var _this5 = this;return _asyncToGenerator(function* () {const preludeId = _this5._getModuleId('__prelude__'); // Get all the polyfills from the relevant option params (the
      // `getPolyfills()` method and the `polyfillModuleNames` variable).
      const polyfillModuleNames = _this5._getPolyfills({ platform: _this5._bundleOptions.platform }).concat(_this5._polyfillModuleNames); // Build the module system dependencies (scripts that need to
      // be included at the very beginning of the bundle) + any polifyll.
      const modules = [defaults.moduleSystem].concat(polyfillModuleNames).map(function (polyfillModuleName) {return _this5._dependencyGraph.createPolyfill({ file: polyfillModuleName, id: polyfillModuleName, dependencies: [] });});const edges = yield Promise.all(modules.map(function (module) {return _this5._createEdgeFromScript(module, transformOptions);}));const transformedModules = yield _this5._transformModules(edges, transformOptions, dependencyEdges); // The prelude needs to be the first thing in the file, and the insertion
      // order of entries in the Map is significant.
      return new Map([[preludeId, _this5._getPrelude(preludeId)]].concat(_toConsumableArray(transformedModules)));})();}_getPrelude(id) {const code = getPreludeCode({ isDev: this._bundleOptions.dev });const name = '__prelude__';return { code, id, map: [], name, source: code, path: name, type: 'script' };}_getAppend(dependencyEdges) {var _this6 = this;return _asyncToGenerator(function* () {// Get the absolute path of the entry file, in order to be able to get the
      // actual correspondant module (and its moduleId) to be able to add the
      // correct require(); call at the very end of the bundle.
      const entryPointModulePath = _this6._dependencyGraph.getAbsolutePath(_this6._bundleOptions.entryFile); // First, get the modules correspondant to all the module names defined in
      ...
      const append = new Map(_this6._bundleOptions.runBeforeMainModule.concat(entryPointModulePath).filter(function (path) {return dependencyEdges.has(path);}).map(_this6._getModuleId).map(function (moduleId) {const code = `require(${JSON.stringify(moduleId)});`;const name = 'require-' + String(moduleId);const path = name + '.js';return [moduleId, { code, id: moduleId, map: [], name, source: code, path, type: 'require' }];}));if (_this6._bundleOptions.sourceMapUrl) {const code = '//# sourceMappingURL=' + _this6._bundleOptions.sourceMapUrl;const id = _this6._getModuleId('/sourcemap.js');append.set(id, { code, id, map: [], name: 'sourcemap.js', path: '/sourcemap.js', source: code, type: 'comment' });}return append;})();}_transformModules(modules, transformOptions, dependencyEdges) {var _this7 = this;return _asyncToGenerator(function* () {return new Map((yield Promise.all(modules.map(function (module) {return _this7._transformModule(module, transformOptions, dependencyEdges);}))));})();}_createEdgeFromScript(module, transformOptions) {return _asyncToGenerator(function* () {const result = yield module.read(transformOptions);const edge = { dependencies: new Map(), inverseDependencies: new Set(), path: module.path, output: { code: result.code, map: result.map, source: result.source, type: 'script' } };return edge;})();}_transformModule(edge, transformOptions, dependencyEdges) {var _this8 = this;return _asyncToGenerator(function* () {const name = _this8._dependencyGraph.getHasteName(edge.path);const dependencyPairs = edge ? edge.dependencies : new Map();let wrappedCode; // Get the absolute path of each of the module dependencies from the
